...
program define ipdmetan, rclass
	...
	** Test for which command structure is being used
	...
	if `rc' {
		...
		if "`cmdstruc'" == "generic" {
			...
			if trim(itrim(`"`invlist'"'))==`""' nois disp as err `"but {it:varlist} has not been supplied."'
			else {
	...
	************************************
	...
	marksample touse, novarlist						// don't lose missing values yet
	...
	local 0 `by'
	syntax [name(name=by)] [, Missing FORCE /*ADDED AUG 2019 FOR v3.3*/ ]		// only a single (var)name is allowed
	...
	* "Forestplot options" are prioritised over "ipdmetan options" in the event of a clash.
	...
	// cumulative, efficacy, influence
	// counts, group1, group2 (for compatibility with metan.ado)
	// rfdist, rflevel (for compatibility with metan.ado)	

	// April 2021:
	// `interaction' is a special case:
	// - as a main option to -ipdmetan- , it specifies that an interaction coefficient be sought and pooled.
	// - as a forestplot suboption, it specifies that "interaction style" be used in the plot, i.e. circles rather than diamonds.
	// This enables analyses to be run in which the *model* is not explicitly an interaction model,
	//  but we want the output (table and forest plot) to be presented as if it were. (e.g. if using -vwls- )
	...
	// N.B. At this stage we also want to isolate the effect MEASURE, to be sent to -metan- in the summstat() option.
	// (METHODs of analysis are not dealt with here, but in -metan- )
	...
	local opts_ipdm  `"`s(opts_parsed)' `s(options)'"'		// options as listed above, plus other options supplied directly to -metan-
	local opts_fplot `"`s(opts_fplot)'"'					// other options supplied as sub-options to forestplot() 	

	// April 2021: special case
	if `"`ipdover'"'!=`""' {
		local int_text `s(int_text)'
	}	

	** Now, if one observation per study, pass directly to -metan- ...
	else {
		...
		if r(r)==r(N) & "`cmdstruc'"=="specific" {
			...
			cap nois metan_setup `invlist' if `touse', study(`studyopt') by(`byopt') ///
				...
				effect(`effect') summstat(`summstat') `eform' `log' `clear' `clearstack' `opts_ipdm'

			if _rc {
				if `"`err'"'==`""' {
					if _rc==1 nois disp as err `"User break in {bf:ipdmetan.metan_setup}"'
					else nois disp as err `"Error in {bf:ipdmetan.metan_setup}"'
				}
				exit _rc
			}
		...
		// If not sending directly to -metan-, default sorting is `study'
	...
	** Option compatibility tests relevant to -ipdover- and "generic" vs "specific" effect measure
	// (N.B. leave more specific MA-related compatibility tests to -metan- )
	...
	if `"`cmdstruc'"'==`"generic"' {
		local 0 `", `opts_ipdm'"'
		syntax [, MH PETO LOGRank COHend HEDgesg GLAssd noSTANdard * ]		// N.B. `counts' has already been parsed [May 2018: ...and `group1', `group2' removed]
		if trim(`"`mh'`peto'`logrank'`cohend'`hedgesg'`glassd'`standard'`counts'"') != `""' {
			local erropt : word 1 of `mh' `peto' `logrank' `cohend' `hedgesg' `glassd' `standard' `counts'
	...
	*******************
	...
	cap nois ProcessIDs if `touse', study(`studyopt') studyid(`StudyID') by(`_BY') obs(`obs') ///
		...
		sortby(`sortby') cmdstruc(`cmdstruc') plname(`plname') `ipdover' `force' /*ADDED AUG 2019 FOR v3.3*/
	...
	*/	
	...
	local _STUDY = cond(`"`ipdover'"'!=`""', `"`prefix'_LEVEL"', `"`prefix'_STUDY"')
	...
	if trim(`"`lcols'`rcols'"')==`""' | (`"`saving'"'==`""' & `"`graph'"'!=`""' & `"`citype'"'==`"normal"') {
		...
		* Get total number of "items" and loop, perfoming housekeeping tasks for each item
		...
		forvalues i=1/`ni' {
			...
			// form new `lcols' and `rcols', just containing new varnames (to pass to forestplot, via -metan- or -ipdover- )
			...
			if "`itype'"=="a" {								// a: AD-only vars, not currently in memory
				local ++na
				local namesa `namesa' `coli'				// AD varlist, to be passed on to -metan-
			}
	...
	* (2) User-defined weights
	if `"`wgt'"'!=`""' {
		...
		if "`cmdstruc'"=="specific" {
			...
			if _rc {
				nois disp as err "Error in option {bf:wgt()}"
	...
	* (3) Parse `plotid'
	...
	if `"`ipdover'"'!=`""' {
		if "`plname'" != "" {
			...
			if "`plname'"=="_BY" & "`by'"=="" {
				disp `"{error}Note: {bf:plotid(_BY)} cannot be specified without {bf:by()} and will be ignored"'
		...
		if "`plname'"=="_BYAD" | ("`plname'"=="`by'" & "`by'"!="" & `by_rc') {		// either _BYAD, or `by' in AD only
			if "`plname'"=="_BYAD" & "`ad'"=="" {
				disp `"{error}Note: {bf:plotid(_BYAD)} cannot be specified without aggregate data and will be ignored"'
				local plotid
			}
	...
	if `"`plotid'"'!=`""' {																	// `plotid' not needed anymore in -ipdmetan- ...
		if `"`ipdover'"'==`""' local opts_ipdm  `"`macval(opts_ipdm)'  plotid(`plotid')"'	// if passing to -metan- , add to `opts_ipdm'...
	...
	**********
	...
	if "`rsample'" != "" {
		...
		if !_rc {
			...
			disp `"{error}Note that this variable is therefore NOT associated with the most recent analysis."'
		}
	}
	qui count
	local origN = r(N)
	...
	* "command"-based syntax (Syntax 1, see help file)
	if "`cmdstruc'"=="generic" {
		...
		cap nois `version' CommandLoop `exp_list' if `touse', ///
			...
			by(`_BY') `ipdover' `interaction' `overallopt' `subgroup' `total' `rsample' prefix(`prefix') ///
			overlen(`overlen') `overtype' statsr(`statsr') namesr(`namesr') `level' `messages' `ztol' `strata_opt'
		...
		local outvlist `prefix'_ES `prefix'_seES
		preserve
	...
	* "Specific" effect measures; "collapse"-based syntax
		...
		** For this syntax, how the data is converted from IPD to AD depends on what sort of data it is,
		...
		// If `summstat' does not yet exist, check remaining options for iv, mh, peto, logrank, cohend, hedgesg, glassd, (no)standard
		...
		// (N.B. A full parse of `method' will be done by -metan- )
		
		local 0 `", `opts_ipdm'"'
		syntax [, IV MH PETO LOGRank COHend HEDgesg GLAssd noSTANdard NPTS(string) BYAD VARS(string) * ]
		local opts_ipdm : copy local options
		...
		if `"`summstat'"'!=`""' {
			cap {
				if trim(`"`cohend'`glassd'`hedgesg'"')!=`""' assert `"`summstat'"'==`"smd"'
			...
			else if trim(`"`cohend'`glassd'`hedgesg'"')!=`""' {
				local summstat smd
		...
		local opts_ipdm = trim(`"`macval(opts_ipdm)' `iv' `mh' `peto' `logrank' `cohend' `hedgesg' `glassd' `standard'"')
		// now logrank has potentially been altered, put all these options back into `opts_ipdm' for passing to -metan-
		...
		* Setup `outvlist' ("output" varlist, to become the *input* into -metan-, or to be returned to -ipdover-)
	...
	// Finalise "ipdmetan" options to send to "metan_setup"
	local lrvlist = cond(`"`logrank'"'==`""', `""', `"`lrvlist'"')		// in case `lrvlist' passed from -ipdover-
	// local opts_adm `"estexp(`estexp') explist(`exp_list') `interaction' extraline(`extraline') lrvlist(`lrvlist')"'
	...
	*** Perform -collapse- on `cclist' supplied to lcols/rcols, plus processing of raw data
	...
	if `"`cmdstruc'"'==`"specific"' | trim(`"`cclist'`svars'"') != `""' {
		...
		if `"`cmdstruc'"'==`"specific"' | trim(`"`cclist'"') != `""' {		// i.e. all except if `svars' alone
			forvalues h=1/`overlen' {
				...
				if `"`cmdstruc'"'==`"specific"' {
					cap nois ProcessRawData `invlist' if `touse', ///
						...
						lrvlist(`lrvlist') coldnames(`coldnames') strata(`strata') `show' `prefix'
			...
			if `"`overall'"'==`""' {
				...
				if trim(`"`cclist'`r(cclist)'"')!=`""' {
					...
					qui save `extra1_tot'														//   ...this will be changed within ProcessAD)
				}
				restore, preserve
			}
		...
		* Append files to form a single "extra" file
		...
		if `overlen' > 1 {								// if "over", append files
			...
			forvalues h = 2 / `overlen' {
				...
				qui replace `prefix'_OVER = `h' if missing(`prefix'_OVER)
			}
			local _OVER `prefix'_OVER
		...
		* Apply variable labels to "collapse" vars
		...
		if `"`_BY'"'!=`"`prefix'_BY"' & `"`_BY'"'!=`""' {
			rename `_BY' `prefix'_BY			// avoid using "capture" in case of genuine error
			local _BY `prefix'_BY
		}
		
		if `"`cmdstruc'"'==`"generic"' {
			tempvar merge
			qui merge 1:1 `_BY' `_OVER' `_STUDY' using `ipdfile', assert(match using) gen(`merge')
			qui assert inlist(`prefix'_USE, 1, 2) if `merge'==3 & `tempuse'==1
			qui assert `prefix'_USE==`tempuse' if `merge'==3 & `tempuse'!=1
			qui assert inlist(`prefix'_USE, 3, 5) if `merge'==2		// N.B. only applies if no cclist (subgroup/overall not applicable for svars alone)
			qui drop `tempuse' `merge'
		}
		else qui rename `tempuse' `prefix'_USE
	...
	if `"`_BY'"'!=`""' {
		...
		label var `prefix'_BY `"`byvarlab'"'

		if `"`bylabfile'"'!=`""' {
			qui do `bylabfile'
			cap label drop `prefix'_BY
			label copy `bylab' `prefix'_BY
			label values `prefix'_BY `prefix'_BY
		}
	}
	...
	** Raw data post-"collapse" tidying:
	if `"`cmdstruc'"'==`"specific"' {
		...
		// _NN for logrank
			...
			qui gen long `prefix'_NN = `n0' + `n1'	// total numbers of patients by study
	...
	*******************************
	* Pass to -metan- for pooling *  -- or prepare to return data to -ipdover-
	*******************************
	...
	// Branch by -metan- or -ipdover-
	if `"`ipdover'"'==`""' {
	
		if `"`StudyID'"'!=`"`prefix'_STUDY"' qui drop `StudyID'
		confirm numeric var `prefix'_STUDY
		label values `prefix'_STUDY `vallab1'
		label variable `prefix'_STUDY `"`svarlab'"'
		format `prefix'_STUDY `studyfmt'				// ADDED DEC 2020
		...
		cap nois metan_setup `outvlist', study(`studyopt') by(`byopt') citype(`citype') ///
			...
			estexp(`estexp') explist(`exp_list') extraline(`extraline') lrvlist(`lrvlist') `clear' `clearstack' `opts_ipdm'
	...
	*** -ipdover- stuff ***
	...
	// Else, more processing is required to obtain _ES, _seES, _LCI and _UCI before passing back to -ipdover-
	// (N.B. these are mostly processes otherwise done by -metan- )
	else {
		...
		if `"`cmdstruc'"'==`"specific"' {
			...
			return local invlist  `outvlist'				// N.B. `outvlist' becomes `invlist' for clearer comparison with -metan- code
...
program define ProcessCommand, sclass
	...
	local NOTALLOWED by bysort mi sw stepwise bayes collect
	...
	while `"`before'"'!=`""' {
		...
		else if !`:list cmd in YESALLOWED' {
...
program define ProcessIDs, rclass sortpreserve
	...
	syntax [if] [in], STUDY(string) CMDSTRUC(string) LABFILE(string) OBS(name) TVLIST(namelist) TNLIST(namelist) ///
		[BY(varname) SORTBY(varname) PLNAME(varname) IPDOVER STUDYID(name) FORCE /*ADDED Aug 2019 for v3.3*/ ]
	...
	forvalues h=1/`overlen' {
		...
		if `"`ipdover'"'==`""' {
			...
			*/
			...
			// Check for "deluded" meta-analysis: subgroup variable varies within studies
			if `"`force'"'==`""' {
				qui tab `overh' if `stouse', m
				if r(r) != `ns' {					// N.B. `ns' is already stratified by `by'
					nois disp as err `"Data is not suitable for meta-analysis"'
					nois disp as err `"  as variable {bf:`by'} (in option {bf:by()}) is not constant within studies."'
					nois disp as err `"Use either the (undocumented) option {bf:by(}{it:varname}{bf:, force)}, or the alternative command {bf:ipdover}."'
					nois disp as err `"For more details on the reasoning behind this, see:"'
					nois disp as err `"  {browse "https://www.bmj.com/content/356/bmj.j573":Fisher DJ et al. BMJ 2017; 356: j573}"'
					exit 198
				}
			}
			...
			if `"`plname'"'!=`""' {
				if `"`force'"'!=`""' {
					local byopt `by'
					local addtext " and subgroups"
				}
				tempvar tempgp
				qui bysort `stouse' `plname' `overh' `byopt' : gen long `tempgp' = (_n==1)*`stouse'
				qui replace `tempgp' = sum(`tempgp')
				summ `tempgp', meanonly
				local ntg = r(max)
				drop `tempgp'
				...
				if `ntg' > `ns' {
					nois disp as err `"variable {bf:`plname'} in option {bf:plotid()} is not constant within studies`addtext'"'
					exit 198
				}
			}
		...
		* If any string variables, "decode" them
		...
		if !_rc {
			...
			if `"`levels'"'!=`""' {
				...
				*/
				// Modified Aug 2020 to preserve "extra" label mappings (i.e. values don't appear in IPD)
				//  because they might appear in AD instead... e.g. there might be an agreed "project-wide" label mapping
				if `"`: value label `overh''"'==`""' {		// if no existing label
					foreach x of local levels {
						if `x' != . {				// cannot label "."
							local labname : label (`overh') `x'
							label define `vallab`h'' `x' `"`labname'"', add
					...
					label copy `: value label `overh'' `vallab`h''
					foreach x of local levels {
						if `x' != . {				// cannot label "."
							local labname : label (`overh') `x'
							if `"`labname'"'==`""' {
								label define `vallab`h'' `x' `"`x'"', add
							}
						}
					}
				}
				local miss .
				local levels : list levels - miss
				if `"`levels'"'!=`""' local lablist `"`lablist' `vallab`h''"'
			}
		}
...
program define ParseFPlotOpts, sclass
	...
	** Now parse "forestplot options" if applicable
	...
	if `"`forestplot'"'!=`""' {
		...
		//  (N.B. `interaction' should be part of `optlist1', but needs to be treated slightly differently)
		local optlist `optlist1' `optlist2' `optlist3' effect interaction
		...
		// April 2021: `interaction' is a special case
		if `"`interaction'"'!=`""' | `"`interaction_main'"'!=`""' {
			local int_text interaction			// for -ipdover- only
		}
		if `"`interaction'"'!=`""' & `"`interaction_main'"'==`""' {
			local opts_fplot `"`macval(opts_fplot)' interaction"'
		}
		else local optlist1 `optlist1' interaction		// treat as usual
	...
	** Finalise locals & scalars as appropriate; forestplot options take priority
	...
	if `"`forestplot'"'==`""' {		// April 2021
		local opts_parsed `"`macval(opts_parsed)' `interaction'"'
	}
	...
	sreturn local int_text `int_text'		// for -ipdover- only
...
program define CommandLoop, rclass sortpreserve
	...
	** If appropriate, generate blank subgroup observations
	...
	// Warning messages
	if `"`total'"'!=`""' {
		disp _n `"{error}Caution: initial model fitting in full sample was suppressed"'
	}
	if `"`pcommand'"'!=`""' {
		disp _n `"{error}Caution: prefix command supplied to {bf:ipdmetan}. Please check estimates carefully"'
	}
	if `noconverge' {
		disp _n `"{error}Caution: model did not converge for one or more studies. Pooled estimate may not be accurate"'
	}
	if `userbreak' {
		disp _n `"{error}Caution: model fitting for one or more studies was stopped by user. Pooled estimate may not be accurate"'
	}
...
* Prepare data for sending to -metan-
...
program define metan_setup, rclass
	...
	syntax varlist(min=2 max=6 numeric) [if] [in], STUDY(string) [BY(string) ///
		...
		EFORM LOG LOGRank EFFECT(string) SUMMSTAT(string) NPTS(varname) NPTS2 noINTeger noKEEPVars noRSample CLEAR * ]
	...
	*/
	...
	if "`cmdstruc'"!="" {
		...
		local _STUDY `prefix'_STUDY

		local _BY
		cap confirm numeric var `prefix'_BY
	...
	//   with user-defined (var)names (but e.g. _BY may not exist yet; only in AD)
		...
		if `"`ad'"'!=`""' {		// within "`cmdstruc'"==""
			...
			cap nois ProcessLabelsIPD if `touse', ///
				study(`study') newstudy(`newstudy') newstudylab(`newstudylab') ///
				   by(`by')       newby(`newby')       newbylab(`newbylab')
			if _rc {
				if _rc==1 nois disp as err `"User break in {bf:ipdmetan.ProcessLabels} when applied to IPD"'
				else if _rc!=2000 nois disp as err `"Error in {bf:ipdmetan.ProcessLabels} when applied to IPD"'
				exit _rc
			}
			
			if `"`s(newstudy)'"'!=`""' {
				local _STUDY `s(newstudy)'
				label values `_STUDY' `s(newstudylab)'
				label variable `_STUDY' `"`svarlab'"'
			}
			if `"`s(newby)'"'!=`""' {
				local _BY `s(newby)'
				label values `_BY' `s(newbylab)'
	...
	if `"`ad'"'!=`""' {
		...
		if _rc {
			if _rc==2000 {
				disp `"{error}Note: No valid observations in aggregate dataset; {bf:ad()} will be ignored"'
				cap drop `_SOURCE'
				local _SOURCE
			}
			else {
				...
				c_local err noerr		// tell ipdmetan not to also report an "error in {bf:metan_setup}"
				exit _rc
			}
		}
	}		// end if `"`ad'"'!=`""'
	...
	if `"`_SOURCE'"'!=`""' {
		...
		if `"`s(effect)'"'!=`""' local effect `"`s(effect)'"'
		if `"`summstat'"'==`""' local summstat `s(summstat)'
	
		local eform   `s(eform)'
		local logrank `s(logrank)'
		...
		disp as text _n "Studies included from IPD: " as res `s(kIPD)'
		if "`keepall'"!="" & `s(kIPD_2)' {
			local plural = cond(`s(kIPD_2)'==1, "study", "studies")
			disp as text "  plus " as res `s(kIPD_2)' as text " `plural' with insufficient data"
		}		
		local dispnpts = cond(missing(`s(nIPD)'), "Unknown", string(`s(nIPD)'))
		disp as text "Participants included from IPD: " as res "`dispnpts'"
		if "`keepall'"!="" & `s(kIPD_2)' {
			local dispnpts = cond(missing(`s(nIPD_2)'), "Unknown", string(`s(nIPD_2)'))
			local s = cond(`s(nIPD_2)'>1, "s", "")
			disp as text "  plus " as res "`dispnpts'" as text " participant`s' with insufficient data"
		}
		
		disp as text _n "Studies included from aggregate data: " as res `s(kAD)'
		if "`keepall'"!="" & `s(kAD_2)' {
			local plural = cond(`s(kAD_2)'==1, "study", "studies")
			disp as text "  plus " as res `s(kAD_2)' as text " `plural' with insufficient data"
		}
		local dispnpts = cond(missing(`s(nAD)'), "Unknown", string(`s(nAD)'))
		disp as text "Participants included from aggregate data: " as res "`dispnpts'"
		if "`keepall'"!="" & `s(kAD_2)' {
			local dispnpts = cond(missing(`s(nAD_2)'), "Unknown", string(`s(nAD_2)'))
			local s = cond(`s(nAD_2)'>1, "s", "")
			disp as text "  plus " as res "`dispnpts'" as text " participant`s' with insufficient data"
		}
	...
	// PREVIOUS NOTES:
	// N.B. `summstat' and `log' are *not* passed to -metan-
	...
	cap nois metan `invlist' if `touse', study(`_STUDY', m) `byopt' `nptsopt' ///
		...
		`keepall' `interaction' `extraline' `clear' `clearstack' `opts_ipdm' ///
		///
...
// subroutine of metan_setup

program define ProcessAD, sclass
	...
	* If AD file exists, prepare data and load file
	...
	if `"`by'"'!=`""' {
		if `"`byad'"'!=`""' {
			disp `"{error}Note: Cannot specify both {bf:byad} and {bf:by()}; {bf:byad} will be ignored"' 
			local byad
		}
	...
	if !_rc {
		...
		*/
		...
		disp `"{error}Note: variable {bf:`study'} (in option {bf:study()}) not found in aggregate dataset;"'
		disp `"{error}      all valid observations in the aggregate dataset will be included"' 
		local study
	}
	...
	//   but if *numeric*, compare value labels and detect clashes
		...
		if !_rc {
			...
			if `"`: list ipd_slist & ad_slist'"'!=`""' {	// clash in original IPD and AD study values; new label needed
				if `"`relabel'"'==`""' {
					if `"`ipdstr'"'==`""' {
						nois disp as err `"Study value label conflict between AD and IPD"'
						nois disp as err `"If appropriate, use the {bf:relabel} suboption of {bf:ad()} to force relabelling of both AD and IPD"'
					}
					else {
						nois disp as err `"Study value label conflict between AD and IPD, due to {bf:study()} being converted from string in IPD."'
						nois disp as err `"To avoid this conflict, supply a numeric (within IPD) variable to the {bf:study()} option;"'
						nois disp as err `"  or to over-ride it, use the {bf:relabel} suboption of {bf:ad()} to force relabelling of both AD and IPD."'
					}
					exit 180
				}
				local smax : word `: word count `ipd_slist'' of `ipd_slist'	// max IPD study value
			}
			else local smax = -9	// if `study' is numeric in AD with no clashes; this will cause ProcessLabelsIPD to simply merge the labels
		}
		else local smax = `ns'
	}
	...
	* Now, create common labels for `study' and `by' across IPD and AD datasets
	...
	cap nois ProcessLabelsIPD if `touse', smax(`smax') nby(`nby') `relabel' ///
		study(`study') newstudy(`_STUDY') newstudylab(`studylab') ///
		by(`by_in_AD') newby(`_BY')       newbylab(`bylab')
	if _rc {
		if _rc==1 nois disp as err `"User break in {bf:ipdmetan.ProcessLabels} when applied to AD"'
		else if _rc!=2000 nois disp as err `"Error in {bf:ipdmetan.ProcessLabels} when applied to AD"'
		c_local err noerr			// tell -ipdmetan- not to also report an "error in ProcessAD"
		exit _rc
	}
		
	// update `study' within AD; don't apply label just yet though
	if `"`s(newstudy)'"'==`""' {									// if r(newstudy) not returned, just use the original variable
		if `"`study'"'!=`"`_STUDY'"' qui rename `study' `_STUDY'	// (we can rename it since we are under -preserve-)
	}
	else if `"`s(newstudy)'"'!=`"`_STUDY'"' {	// if r(newstudy) *was* returned, use it
		cap drop `_STUDY'						// "capture" since `_STUDY' might not exist
		qui rename `s(newstudy)' `_STUDY'		// (N.B. can safely drop, since `_STUDY' can't be in lcols/rcols)
	}
	...
	if "`by_in_AD'"!=`""' {
		...
		if `"`s(newbylab)'"'!=`""' local bylab `s(newbylab)'
		if `"`s(newby)'"'!=`""'    local by_in_AD `s(newby)'
	...
	* Extra work specific to *separate* AD file
	if `"`adfile'"'!=`""' {
		...
		if `"`wgt'"'!=`""' {
			...
			if _rc {
				disp `"{error}Note: user-defined weights {bf:`wgt'} not found in aggregate dataset"'
				local wgt
			}
		}
	...
	* Sort out `npts' in AD dataset;
	...
	if "`adnpts'"!="" {
		...
		// local s_npts : copy local npts	// for sreturn (`adnpts' is not needed henceforth)
	}
	if "`npts'"!="" local npts_opt `"npts(`npts')"'		// for passing to -metan-
	...
	************************************
	...
	if `adparams'==4 {
		if `"`summstat'"'==`""' {
			...
			disp _n `"{error}Note: Effects assumed to represent `logstr'Risk Ratios, due to 2x2 count data in {bf:ad()}"'
			local summstat rr
		...
		if `"`logrank'"'==`""' {
			local logstr = cond(`"`eform'"'==`""', `"log "', `""')
			disp _n `"{error}Note: Effects assumed to represent `logstr'Hazard Ratios, due to {bf:logrank} suboption in {bf:ad()}"' 
		}
		local summstat hr
	...
	if `adparams'==`params' & "`adlogrank'"=="`ipdlogrank'" {
		...
		if inlist(`"`summstat'"', "hr", "shr") & `"`adlogrank'"'==`""' & `adparams'==2 {
			disp `"{error}Note: Aggregate data variables assumed to represent {it:logHR} and {it:selogHR}"'
			disp `"{error}      If in fact they should represent {it:O-E} and {it:V}, please supply the {bf:logrank} suboption to {bf:ad()}"' 
		}
	}
	...
	// (N.B. this may occur even if both are nominally I-V)
		...
		if `"`logrank'"'==`""' {	// i.e & `"`adlogrank'"'!=`""' {
			disp `"{error}Note: Aggregate data variables assumed to represent {it:O-E} and {it:V} due to {bf:logrank} suboption in {bf:ad()}"'
		}
		else {	// i.e. if `"`adlogrank'"'==`""' & `"`logrank'"'!=`""' {
			disp `"{error}Note: Aggregate data variables assumed to represent {it:logHR} and {it:selogHR}"'
			disp `"{error}      If in fact they should represent {it:O-E} and {it:V}, please supply the {bf:logrank} suboption to {bf:ad()}"' 
			disp `"{error}     (that is, in addition to the {bf:logrank} main option to {bf:ipdmetan})"' 
		}
		// local method iv
	}
	...
	* If necessary, coerce non-IV data into IV format so that entire dataset to be analysed together.
	...
	// Need to run -metan- here, both to coerce non-IV data,
	//  but also to obtain numbers of studies and participants, and details of insufficient data
	// Hence, if `invlist' or `adinvlist' contain _ES, _seES etc., 
	//  rename them so that they are not overwritten by -metan-
	local metanlist _ES _seES _LCI _UCI _WT _NN _rsample
	local invlist2 : copy local invlist
	local adinvlist2 : copy local adinvlist
	foreach v of varlist `invlist' {
		if `: list v in metanlist' {
			tempvar temp`v'
			qui rename `v' `temp`v''
			local invlist2 = subinstr("`invlist2'", "`v'", "`temp`v''", .)
			local adinvlist2 = subinstr("`adinvlist2'", "`v'", "`temp`v''", .)
		}
	}
	foreach v of varlist `adinvlist2' {		// if any still to be done
		if `: list v in metanlist' {
			tempvar temp`v'
			qui rename `v' `temp`v''
			local adinvlist2 = subinstr("`adinvlist2'", "`v'", "`temp`v''", .)
	...
	qui ds
	local allvlist `"`r(varlist)'"'
	local restvlist : list allvlist - invlist
	local restvlist : list restvlist - adinvlist
	foreach v of varlist `restvlist' {		// if any still to be done
		if `: list v in metanlist' {
			tempvar temp`v'
			qui rename `v' `temp`v''
		}
	...
	if "`npts'"=="_NN" {
		local npts `temp_NN'
		local npts_opt `"npts(`temp_NN')"'		// for passing to -metan-
	}	
	
	// Run -metan- on IPD
	cap metan `invlist2' if `_SOURCE'==1 & `_USE'==1 & `touse', `logrank' `npts_opt' nograph
	if _rc {
		if _rc==1 nois disp as err `"User break in {bf:metan}"' _c
		else nois disp as err `"Error in {bf:metan}"' _c
		if !`ipd_iv' nois disp as err `" whilst converting IPD to inverse-variance format"'
		else nois disp as err " "		// break the _c directive
		exit _rc
	}
	local kIPD = r(k)
	local nIPD = r(n)
	
	qui replace `_USE' = 2 if `_SOURCE'==1 & `_USE'==1 & `touse' & _rsample==0	
	qui count if `_SOURCE'==1 & `_USE'==2 & `touse'
	local kIPD_2 = r(N)					// _USE==2
	
	local nIPD_2 = .
	if "`npts_opt'"!="" {
		summ `npts' if `_SOURCE'==1 & `_USE'==2 & `touse', meanonly
		if r(N) local nIPD_2 = r(sum)	// _USE==2
	}
	
	
	** If appropriate, copy data from newly-created IPD vars _ES, _seES (or _LCI, _UCI) into AD varnames (`adinvlist2')
	// NOTE: If IPD is logrank, we want -metan- to analyse OE & V
	//  so that it prints "log Haz. Ratio"; "Peto log-rank" etc. on-screen.
	// Hence, need to convert `adinvlist' to OE & V before passing on to -metan- for final analysis.
	...
	if `ad_iv' & !`ipd_iv' & `"`logrank'"'==`""' {
		tokenize `adinvlist2'
		qui replace `1' = _ES if `_SOURCE'==1 & `_USE'==1 & `touse'
		if `adparams'==2 {
			qui replace `2' = _seES if `_SOURCE'==1 & `_USE'==1 & `touse'
		}
		else {
			qui replace `2' = _LCI if `_SOURCE'==1 & `_USE'==1 & `touse'
			qui replace `3' = _UCI if `_SOURCE'==1 & `_USE'==1 & `touse'
		}
		if `"`npts_opt'"'!=`""' {
			qui replace `npts' = _NN if `_SOURCE'==1 & `_USE'==1 & `touse'
		}
	}
	
	// March 2021:
	// Do similar if AD & IPD both I-V, but AD is `params'==2 and IPD is `params'==3
	else if `ipd_iv' & `ad_iv' & `params'==3 & `adparams'==2 {
		tokenize `adinvlist2'
		qui replace `2' = _seES if `_SOURCE'==1 & `_USE'==1 & `touse'
		if `"`npts_opt'"'!=`""' {
			qui replace `npts' = _NN if `_SOURCE'==1 & `_USE'==1 & `touse'
	...
	// Run -metan- on AD
	cap metan `adinvlist2' if `_SOURCE'==2 & `_USE'==1 & `touse', `adlogrank' `npts_opt' nograph
	if _rc {
		if _rc==1 nois disp as err `"User break in {bf:metan}"' _c
		else nois disp as err `"Error in {bf:metan}"' _c
		if !`ad_iv' nois disp as err `" whilst converting aggregate data to inverse-variance format"'
		else nois disp as err " "		// break the _c directive
		exit _rc
	}
	local kAD = r(k)
	...
	qui replace `_USE' = 2 if `_SOURCE'==2 & `_USE'==1 & `touse' & _rsample==0		
	qui count if `_SOURCE'==2 & `_USE'==2 & `touse'
	local kAD_2 = r(N)					// _USE==2
	
	local nAD_2 = .
	if "`npts_opt'"!="" {
		summ `npts' if `_SOURCE'==2 & `_USE'==2 & `touse', meanonly
		if r(N) local nAD_2 = r(sum)	// _USE==2
	}

	
	// If IPD is logrank (see explanation above)
	if `ad_iv' & !`ipd_iv' & `"`logrank'"'!=`""' {
		tokenize `invlist2'
		args oe v
		qui replace `v'  = 1 / _seES^2 if `_SOURCE'==2 & `_USE'==1 & `touse'
		qui replace `oe' = `v' * _ES   if `_SOURCE'==2 & `_USE'==1 & `touse'
	}
	
	// Else, if appropriate, copy newly-created data from _ES, _seES (or _LCI, _UCI) into IPD varnames (`invlist2')
	if !`ad_iv' & `ipd_iv' {
		tokenize `invlist2'
		qui replace `1' = _ES if `_SOURCE'==2 & `_USE'==1 & `touse'
		if `params'==2 {
			qui replace `2' = _seES if `_SOURCE'==2 & `_USE'==1 & `touse'
		}
		else {
			qui replace `2' = _LCI if `_SOURCE'==2 & `_USE'==1 & `touse'
			qui replace `3' = _UCI if `_SOURCE'==2 & `_USE'==1 & `touse'
		}
		if `"`npts_opt'"'!=`""' {
			qui replace `npts' = _NN if `_SOURCE'==2 & `_USE'==1 & `touse'
		}
	}
		
	// March 2021:
	// Do similar if AD & IPD both I-V, but IPD is `params'==2 and AD is `params'==3
	else if `ipd_iv' & `ad_iv' & `adparams'==3 & `params'==2 {
		tokenize `invlist2'
		qui replace `2' = _seES if `_SOURCE'==2 & `_USE'==1 & `touse'
		if `"`npts_opt'"'!=`""' {
			qui replace `npts' = _NN if `_SOURCE'==2 & `_USE'==1 & `touse'
	...
	foreach v in `metanlist' {
		cap drop `v'
		cap confirm variable `temp`v''
		if !_rc {
			qui rename `temp`v'' `v'
		}
	}
	...
	** Sort out various permutations of _BY
	...
	// If `byad', generate _BY variable which is a clone of _SOURCE
	else if `"`byad'"'!=`""' {
		qui gen byte `newby' = `_SOURCE'
		label copy `sourcelab' `newbylab'
		label values `newby' `newbylab'
		label variable `newby' "Data source"
		local _BY `newby'
	}
	...
	// Return values
	sreturn clear
	sreturn local invlist  `invlist'
	sreturn local nptsvar  `npts'
	sreturn local by       `_BY'
	sreturn local byad     `byad'
	sreturn local eform    `eform'
	sreturn local effect `"`effect'"'
	sreturn local logrank  `logrank'
	// sreturn local method   `method'
	sreturn local summstat `summstat'

	sreturn local kIPD `kIPD'
	sreturn local nIPD `nIPD'
	sreturn local kIPD_2 `kIPD_2'
	sreturn local nIPD_2 `nIPD_2'
	
	sreturn local kAD `kAD'
	sreturn local nAD `nAD'
	sreturn local kAD_2 `kAD_2'
	sreturn local nAD_2 `nAD_2'
...
//    named ProcessLabelsIPD to differentiate from similar subroutine in metan.ado)
...
program define ProcessLabelsIPD, sclass sortpreserve
	...
	** Subgroup (`by') labelling (if applicable)
	...
	// If IPD alone, ProcessLabelsIPD will never be run.
	...
	if `"`by'"'!=`""' {
		...
		if !_rc & `nby' {
			...
			local newby `by'
			sreturn local newby    `by'			// use existing variable...
			sreturn local newbylab `newbylab'	// ...but new (extended from IPD) value label
		}
		...
		// string
			...
			label values `newby' `newbylab'
			sreturn local newby    `newby'
			sreturn local newbylab `newbylab'
		}
	...
	** Study label
	...
	if !`noloop' {
		...
		if `rc' | `"`relabel'"'!=`""' {
			...
			sreturn local newstudy `newstudy'
		}
		...
		forvalues i = 1 / `nsad' {
			...
			if `"`study'"'==`""' {
				...
				// similarly, if `study' is numeric but unlabelled, store raw values in `newstudylab'
				else {
					cap assert `"`: label `newstudylab' `si_new''"'==`"`si_old'"'			// if label is already defined correctly, don't attempt to re-label
					if _rc cap label define `newstudylab' `si_new' `"`si_old'"', add		// Aug 2020: "cap" added for numeric-but-unlabelled `study' ...
				}																			// ... since `newstudylab' may already exist with pre-defined study labels
			}
		}

		sreturn local newstudylab `newstudylab'
	}
